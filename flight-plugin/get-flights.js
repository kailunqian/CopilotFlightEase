...
export default function getFlights(departure_airport, destination_airport, departure_city, destination_city, date, dateRangeStart, dateRangeEnd, stops, flightClass) {
  let filteredFlights = flights.filter(flights => {
      ...
      const stopsMatch = stops>=0 ? flights.stops <= stops : true;

      const flightClassMatch = flightClass
      ? flights.class.toLowerCase() == flightClass.toLowerCase()
      : true;

      return departure_airport_match && destination_airport_match && departure_city_match && destination_city_match && departure_date_match && stopsMatch && flightClassMatch;
    }).sort((a, b) => {
      return a.price - b.price;
    }).slice(0, RESULT_LIMIT);

    filteredFlights.forEach(flight => {
      flight.events = events.filter(event => {
        return new Date(event.meetingStart).getDate() === new Date(flight.departure_time).getDate();
      });
    });
